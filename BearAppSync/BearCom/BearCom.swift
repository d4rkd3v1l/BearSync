...
class BearCom {
    ...
    func create(with text: String, for fileId: FileId) async throws -> CreateResult {
        let requestId = UUID()
        let url = URL(string: "bear://x-callback-url/create?text=\(text)&open_note=no&show_window=no&x-success=bearappsync://x-callback-url/create?requestId%3d\(requestId)&x-error=bearappsync://x-callback-url/create?requestId%3d\(requestId)")!
        NSWorkspace.shared.open(url)
    ...
    func addText(_ text: String, to noteId: NoteId) async throws -> AddTextResult {
        let requestId = UUID()
        let url = URL(string: "bear://x-callback-url/add-text?id=\(noteId)&text=\(text)&mode=replace_all&open_note=no&show_window=no&x-success=bearappsync://x-callback-url/add-text?requestId%3d\(requestId)")!
        NSWorkspace.shared.open(url)
